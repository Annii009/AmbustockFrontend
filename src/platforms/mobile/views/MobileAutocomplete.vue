<template>
  <div class="m-ac" ref="container">
    <input v-model="query" class="m-ac__input" :placeholder="placeholder" autocomplete="off" inputmode="text"
      @input="onInput" @keydown="onKeydown" @focus="showDropdown = filtered.length > 0" @blur="onBlur" />

    <div v-if="showDropdown && filtered.length > 0" class="m-ac__dropdown">
      <div v-for="(item, i) in filtered" :key="item" class="m-ac__item"
        :class="{ 'm-ac__item--active': i === activeIndex }" @mousedown.prevent="select(item)"
