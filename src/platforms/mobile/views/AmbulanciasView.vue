...
.back-btn, .add-btn {
  ...
  svg { width: 22px; height: 22px; color: $primary-red; }
  &:hover { background: $bg-page; }
}

.add-btn {
  background: $primary-red; border-radius: 8px;
...
.search-box {
  ...
  background: $bg-page; border-radius: 12px;
  padding: 10px 14px; border: 1.5px solid transparent;
  transition: border-color 0.15s;
  svg { width: 17px; height: 17px; color: $text-gray; flex-shrink: 0; }
  input {
    flex: 1; border: none; background: transparent; outline: none;
    font-family: $font-primary; font-size: 14px; color: $text-dark;
    &::placeholder { color: $text-gray; }
  }
  &:focus-within { border-color: $primary-red; background: $white; }
}
...
.spinner {
  width: 28px; height: 28px; border: 3px solid $border-color;
  border-top-color: $primary-red; border-radius: 50%; animation: spin 0.8s linear infinite;
}

// Empty
.empty {
  display: flex; flex-direction: column; align-items: center; gap: 12px;
  padding: 64px 24px; color: $text-gray;
  svg { width: 56px; height: 56px; opacity: 0.25; }
  p { font-family: $font-primary; font-size: 15px; }
}

// Lista
.list { padding: 4px 16px 24px; display: flex; flex-direction: column; gap: 8px; }

.amb-card {
  display: flex; align-items: center; gap: 12px;
  padding: 14px 16px;
  background: $white; border: 1.5px solid $border-color;
  border-radius: 14px; cursor: pointer; transition: all 0.15s;
  &:active { background: $bg-page; transform: scale(0.99); }
}
...
.amb-card__body { flex: 1; min-width: 0; }

.amb-card__name {
  font-family: $font-primary; font-size: 15px; font-weight: $font-semibold;
  color: $text-dark; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}

.amb-card__mat {
  font-family: $font-primary; font-size: 13px; color: $text-gray; margin-top: 2px;
}

.amb-card__arrow {
  svg { width: 18px; height: 18px; color: $text-gray; }
}
</style>