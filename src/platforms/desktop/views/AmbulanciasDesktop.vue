...
.empty-state {
  ...
  background: $white; border-radius: 14px; border: 1px solid $border-color; color: $text-gray;
  svg { width: 40px; height: 40px; opacity: 0.3; }
  p { font-family: $font-primary; font-size: 14px; }
}

.cards-grid {
  display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 0.875rem;
}

.amb-card {
  display: flex; align-items: center; gap: 0.875rem;
  background: $white; border: 1px solid $border-color; border-radius: 12px;
  padding: 1rem 1.25rem; transition: box-shadow 0.15s;
  &:hover { box-shadow: $shadow-card; }
}

.amb-card__icon {
  width: 40px; height: 40px; border-radius: 10px; background: rgba($primary-red, 0.08);
  display: flex; align-items: center; justify-content: center; flex-shrink: 0;
  svg { width: 20px; height: 20px; color: $primary-red; }
}

.amb-card__body { flex: 1; overflow: hidden; }
.amb-card__name { font-family: $font-primary; font-size: 13.5px; font-weight: $font-bold; color: $text-dark; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.amb-card__matricula { font-family: $font-primary; font-size: 12px; color: $text-gray; margin-top: 2px; }
.amb-card__id { font-family: $font-display; font-size: 15px; color: $text-gray; flex-shrink: 0; }
</style>