  ...
  <div class="page">
    <div class="page__inner">
      ...
      <div class="page__header">
        <div>
          <h1 class="page__title">VER REVISIONES</h1>
          <p class="page__sub">Listado completo de revisiones realizadas</p>
        </div>
      ...
      <div class="table-wrap">
        <div v-if="loading" class="table-empty"><div class="spinner" /></div>
...
const filtered = computed(() =>
  revisiones.value.filter(r => {
    const q = search.value.toLowerCase()
    const matchSearch = !q || (r.nombreAmbulancia || r.matricula || '').toLowerCase().includes(q) || (r.nombreResponsable || '').toLowerCase().includes(q)
...
.page__header { @include flex-between; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem; }
...
.btn-primary {
  ...
  background: $primary-red; color: $white; border: none; border-radius: 10px;
...
.filters__search {
  flex: 1; min-width: 220px; display: flex; align-items: center; gap: 0.625rem;
  padding: 0 0.875rem; background: $white; border: 1.5px solid $border-color;
  border-radius: 10px; transition: border-color 0.15s;
  svg { width: 16px; height: 16px; color: $text-gray; flex-shrink: 0; }
  input { flex: 1; border: none; outline: none; font-family: $font-primary; font-size: 14px; color: $text-dark; padding: 0.625rem 0; background: transparent; &::placeholder { color: $placeholder-color; } }
  &:focus-within { border-color: $primary-red; }
}

.filters__select {
  padding: 0.625rem 2rem 0.625rem 0.875rem; background: $white; border: 1.5px solid $border-color;
  border-radius: 10px; font-family: $font-primary; font-size: 13.5px; color: $text-dark;
...
.table-empty {
  display: flex; flex-direction: column; align-items: center; gap: 0.75rem; padding: 3rem;
  color: $text-gray;
...
.table {
  ...
  thead tr { background: $bg-page; th { padding: 0.75rem 1rem; text-align: left; font-family: $font-primary; font-size: 11px; font-weight: $font-bold; color: $text-gray; letter-spacing: 0.06em; text-transform: uppercase; border-bottom: 1px solid $border-color; } }
}

.table__row { border-bottom: 1px solid $border-color; transition: background 0.12s; &:last-child { border-bottom: none; } &:hover { background: $bg-page; } td { padding: 0.875rem 1rem; } }
...
.table__action {
  ...
  border-radius: 7px; background: transparent; border: 1px solid $border-color; cursor: pointer;
  color: $text-gray; transition: all 0.15s;
