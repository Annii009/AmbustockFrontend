...
import DesktopInspectionLayout from '../layouts/DesktopInspectionLayout.vue' // ✅ ya correcto
...
const sugerencias = computed(() => {
  const q = nombreResponsable.value.trim().toLowerCase()
  if (q.length < 2) return []
  return todosLosResponsables.value.filter(n => n.toLowerCase().includes(q))
})
...
const seleccionarSugerencia = (nombre: string) => { nombreResponsable.value = nombre }
const cerrarSugerencias     = () => { setTimeout(() => {}, 150) }
...
const continuar = () => {
  ...
  if (nombre.length >= 2) {
    ...
    router.push('/principal/revision') // ✅ fix 3 — ruta anidada bajo /principal
  }
}
</script>
      ...
      <div class="card-body">
        ...
        <div class="autocomplete-wrapper">
          ...
          <ul v-if="mostrarLista" class="sugerencias-list">
            <li
              v-for="(nombre, idx) in sugerencias"
              :key="idx"
...
.step-card {
  ...
  box-shadow: 0 1px 8px rgba(0,0,0,0.07); max-width: 580px; overflow: hidden;
}
.card-header {
  display: flex; align-items: center; gap: 16px;
  padding: 22px 26px; border-bottom: 1px solid #f0f1f4;
...
.autocomplete-wrapper { position: relative; margin-bottom: 22px; }
.input-wrapper        { position: relative; }

.custom-input {
  width: 100%; padding: 12px 40px 12px 14px; font-size: 14.5px;
  border: 1.5px solid #d1d5de; border-radius: 8px; background: #fff;
  color: #2d3342; transition: border-color 0.2s; box-sizing: border-box;
  &::placeholder { color: #aab0bc; }
  &:focus { outline: none; border-color: $primary-red; box-shadow: 0 0 0 3px rgba(137,29,26,0.07); }
}

.input-spinner {
  position: absolute; right: 13px; top: 50%; transform: translateY(-50%);
  width: 15px; height: 15px; border: 2px solid #e0e2e7;
...
.sugerencias-list {
  ...
  box-shadow: 0 6px 20px rgba(0,0,0,0.1); list-style: none;
  margin: 0; padding: 4px 0; z-index: 100; max-height: 220px; overflow-y: auto;
}
.sugerencia-item {
  padding: 11px 16px; font-size: 13.5px; color: #2d3342; cursor: pointer; transition: background 0.12s;
  &:hover  { background: #f7f8fa; }
  &:active { background: #f0f1f4; }
  :deep(strong) { color: $primary-red; font-weight: 700; }
}
