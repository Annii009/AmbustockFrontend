...
const ambulanciaLabel = computed(() => {
  ...
  const found = ambulancias.value.find(a => a.idAmbulancia === ambulanciaSeleccionada.value)
  return found ? formatearTextoAmbulancia(found) : '—'
})
...
const cargarAmbulancias = async () => {
  try {
    ...
    error.value = err instanceof ApiError ? err.message : 'Error al cargar ambulancias'
  } finally {
