...
import { getAmbulancias, saveAmbulanciaSeleccionada, type Ambulancia, ApiError } from '@/services/api'
...
const cargarAmbulancias = async () => {
  try {
    ...
    ambulancias.value = await getAmbulancias()
  } catch (err) {
    if (err instanceof ApiError) {
      error.value = err.message
    } else {
      error.value = 'Error al cargar ambulancias'
    }
...
// Formatear texto de ambulancia
const formatearTextoAmbulancia = (ambulancia: Ambulancia): string => {
  if (ambulancia.nombre && ambulancia.nombre.trim() !== '') {
    return ambulancia.nombre.trim()
  } else if (ambulancia.matricula && ambulancia.matricula.trim() !== '') {
    return `Ambulancia ${ambulancia.matricula.trim()}`
  } else {
    return `Ambulancia #${ambulancia.idAmbulancia}`
  }
}
...
<template>
  <div class="seleccion-ambulancia">
    <div class="container">
      <div class="header">
      ...
      <div class="form-content">
        ...
        <div class="select-wrapper">
          ...
          >
            ...
            >
              {{ formatearTextoAmbulancia(ambulancia) }}
            </option>
          </select>
        </div>
      </div>
